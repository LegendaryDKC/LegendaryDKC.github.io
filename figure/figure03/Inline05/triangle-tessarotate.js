...
function printf() {
    numTimesToSubdivide = parseInt(myInput.num.value);
    radius = parseInt(myInput.rotate.value);
    if (numTimesToSubdivide < 0 || numTimesToSubdivide > 7) {
        document.getElementById("Triangle").innerHTML = "数据超出范围！";
        return;
    }
    // <canvas id="gl-canvas" width="512" height="512">你的浏览器不支持HTML5 canvas元素</canvas>
    document.getElementById("Triangle").innerHTML = "<canvas id=\"gl-canvas\"width=\"512\" height=\"512\" onclick=\"initTriangles(" + numTimesToSubdivide + ")\"></canvas>";
    ...
    document.getElementById("Tax").value = x;*/
}

function initTriangles(numTimesToSubdivide,radius){
